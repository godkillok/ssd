...
#选择是和ground true的IOA一样的ancher，并且赋予相应的socore(就是IOA的值)
# =========================================================================== #
def tf_ssd_bboxes_encode_layer(labels,
    ...
    #yref# shape(38*38*4), xref, href, wref#shape(4,)
    #href.size=4 主要是将每个gird中的4个anchor取出，然后求每个ancher的4个顶点，接着求各自的面积
    yref, xref, href, wref = anchors_layer
    ymin = yref - href / 2.
    xmin = xref - wref / 2.
    ymax = yref + href / 2.
    xmax = xref + wref / 2. # shape(38*38*4)
    ...
    def jaccard_with_anchors(bbox):
        ...
        """ #主要是将38*38*4个ancher与当前的要处理的ancher 做比较，bbox=【ymin，xmin，ymax，xmax】
        ...
        # Volumes. inter_vol---shape(38*38*4);vol_anchors---shape(38*38*4)   Union(A,B) = A + B - Inter(A,B)
        inter_vol = h * w
    ...
    def condition(i, feat_labels, feat_scores,
        ...
        """Condition: check label index. 删除一些不大于labels应该的最大值的，也就是不对的标签，可是为啥为产生呢？
        """
        r = tf.less(i, tf.shape(labels))
        return r[0]
  #外面套着的循环就是拿所有groud true去和38*38*4个ancher一一比较，如果第i个groud 与 38*38*4种2的某个几个比较重叠，
    # 那么就保留着这几个ancher IOU的值 ，同时把ground true的四个顶点的值和class label ，直接赋予给那几个ancher,
    # 否则就label赋予为0，IOA为0，四个顶点也为min为0，max为1
    #感觉说白了就是想减少后期比较的工作量，否则需要到处计算
    def body(i, feat_labels, feat_scores,
        ...
        # Mask: check threshold（主要选择重叠大于一定值得） + scores + no annotations + num_classes.
        #选择一个ancher，大于目前feat_soce的
        mask = tf.greater(jaccard, feat_scores)
        # mask = tf.logical_and(mask, tf.greater(jaccard, matching_threshold))
        # 选择一个ancher，这个feat_scres要大于-0.5
        mask = tf.logical_and(mask, feat_scores > -0.5)
        # 选择一个ancher，并且label要符合条件
        mask = tf.logical_and(mask, label < num_classes)
        imask = tf.cast(mask, tf.int64)
        fmask = tf.cast(mask, dtype)
        # Update values using mask.
        # 选择一个ancher，这个ancher 如果满足前面的条件，那么他labal 就是groundtrue的label给过去，否则维持上一轮的结果
    ...
    # Encode features.（现在的中心-原来的中心）/
    #这个部分做的boxing , Bounding Box Regression,这个部分的 计算后groud true的缩放，这个是我们学习的目标
