...
def ssd_losses(logits, localisations,
    ...
    with tf.name_scope(scope, 'ssd_losses'):
        lshape = tfe.get_shape(logits[0], 5) #（batch_size，长，宽，每个格子ancher数目，每个ancher的21个类别的可能性）
        ...
        nmask = tf.logical_and(tf.logical_not(pmask), # 选出不是正例，并且gscore>-0.5的负例子
                               gscores > -0.5)
        fnmask = tf.cast(nmask, dtype)
        nvalues = tf.where(nmask, # 如果是负例子，就把sotfmax的背景的Logistics的结果赋过去，否则为1,也就是负例子的地方用背景的score,正例为1
                           predictions[:, 0],
                           1. - fnmask)
